...
class BST2 {
    ...
    search(value) {
        if (!this.root)
            return "Not found: Empty tree";
        let current = this.root;
        while (current) {
            if (value < current.value) {
                if (!current.left) {
                    return "No left: Not found";
                }
                current = current.left;
            }
            else if (value > current.value) {
                if (!current.right) {
                    return "No right: Not found";
                }
                current = current.right;
            }
            else {
    ...
    treeMaxDiameter(root = this.root) {
        let maxDiameter = 0;
    ...
    sumNodes(node = this.root, getValue) {
        if (!node)
            return 0;
    ...
    lca(valueA, valueB) {
        let current = this.root;
        function findLca(node, valueA, valueB) {
            if (!node)
                return null;
    ...
    isBalancedTree(node = this.root) {
        let current = this.root;
        function balancedTree(node = current) {
            if (!node)
                return 0;
    ...
    isSymetry(node = this.root) {
        if (!node)
            return true;
        function symetry(left, right) {
